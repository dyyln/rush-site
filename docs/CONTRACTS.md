...
`rushRooms` is written by the API after the room veto as 7 ids castle to castle (401 first, 301 last). The plugin also accepts the 5 ids for slots 1 to 5, and a `series.maps[n].rushRooms` wins over the top level one. Slot 3 must be a start room (101 to 104) and slots 1, 2, 4 and 5 mid rooms (201 to 212). See `docs/RUSH-ROOM-VETO.md` and the plugin README.
...
Rush room veto: behind `RUSH_ROOM_VETO.enabled` in `packages/shared/src/config/rush-veto.ts` (off by default, the API env `RUSH_ROOM_VETO` overrides it). When on, a Rush match runs a team-vote ban and pick over the 16 start and mid rooms before allocation. It uses the map veto engine, `vetoes.format = "rush-rooms"`, and `veto_state` and the participant `veto` view carry `kind: "rooms"`. The sequence, extra bans, pick slots and leftover slot are config. Picks fill slots from the picking team's castle (team 0 starts T) and the last room left is the start room, both pending confirmation. When the veto finishes, `matches.rush_rooms` stores seven room ids from T castle to CT castle. They appear on `GET /matches/:id` as `rushRooms` and go through the start request into match.json as `rushRooms`, which the plugin ignores for now.
