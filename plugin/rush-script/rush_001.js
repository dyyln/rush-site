...
// ==== rushsite: room veto (begin) ====
// Not Valve's code. Lets the match plugin set the rooms for slots 1 to 5 from the website veto.
// Two ways in, both used in warmup only:
//   - server console chat, see OnPlayerChat below. This is what the plugin uses.
//   - RunScriptInput rushsite_room_<slot>_<roomId> for each slot 1 to 5, then RunScriptInput rushsite_rooms_apply.
...
// Seven room ids, castles included, or null for Valve's random draw.
var _rushsiteForcedRooms = null;
var _rushsitePending = {};

for (const slot of RUSHSITE_SLOTS) {
	for (const roomId of RUSHSITE_PICKABLE) {
		Instance.OnScriptInput(`rushsite_room_${slot}_${roomId}`, () => { _rushsitePending[slot] = roomId; });
	}
}

Instance.OnScriptInput("rushsite_rooms_apply", () => {
	const picks = RUSHSITE_SLOTS.map((slot) => _rushsitePending[slot]);
	_rushsitePending = {};
	RushsiteApplyRooms(picks);
});

// Second channel, because ent_fire from the server console does nothing on a dedicated server.
// The plugin runs `say rushsite_rooms 203,207,101,211,205` from the server console, and
// `say rushsite_rooms print` logs the rooms in play. Only chat with no player behind it counts,
// so a player typing the same text changes nothing.
Instance.OnPlayerChat((event) => {
	if (event.player) return;
	const match = /^rushsite_rooms\s+(print|[0-9,\s]+)$/.exec(event.text.trim());
	if (!match) return;
	if (match[1] == "print") RushsitePrintRooms();
	else RushsiteApplyRooms(match[1].split(",").map((id) => Number(id.trim())));
});

function RushsiteApplyRooms(picks) {
...
function RushsitePrintRooms() {
	RushsiteLog(`rooms in play ${_roomIds.join(",")}, forced ${_rushsiteForcedRooms ? "yes" : "no"}, current slot ${_currentRoomIndex}`);
}

// Tools mode re-runs this file on save. Carry the forced set over.
Instance.OnScriptReload({
	before: () => _rushsiteForcedRooms,
	after: (memory) => { _rushsiteForcedRooms = memory ?? null; },
});

// Proves the override won over Valve's copy in pak01. Also echoed, because Msg output does not always reach the server log.
function RushsiteLog(text) {
	Instance.Msg(`rushsite: ${text}`);
	Instance.ServerCommand(`echo "rushsite: ${text}"`);
}
RushsiteLog("room veto script loaded");

// Called from RandomizeRooms. A copy, because MaybeSwapInDeciderRoom writes Convoy into _roomIds.
function RushsiteUseForcedRooms() {
	if (!_rushsiteForcedRooms) return false;
	_roomIds = _rushsiteForcedRooms.slice();
	return true;
}
// ==== rushsite: room veto (end) ====
...
function RandomizeRooms() {
	...
	if (RushsiteUseForcedRooms()) return; // rushsite: room veto
