...
export type MetricName = "queue_depth" | "matches_started" | "median_wait_sec" | "active_sockets"
...
export async function sampleMetrics(db: Db, input: SampleInput): Promise<{ written: number; pruned: number }> {
  ...
  const [started, waits] = await Promise.all([
    db
      .select({ mode: matches.mode, n: sql<number>`count(*)::int` })
      .from(matches)
      .where(and(isNotNull(matches.startedAt), gte(matches.startedAt, since), lt(matches.startedAt, minute)))
  ...
  add("matches_started", "", started.reduce((n, r) => n + Number(r.n), 0), since)
...
export async function metricsView(db: Db, range: MetricsRange, now: Date): Promise<MetricsView> {
  ...
  const [lines, bars] = await Promise.all([
    ...
    buckets(db, new Date(barFrom), new Date(barTo), cfg.matchesStepSec, ["matches_started"]),
  ])
  ...
  return {
    ...
    matchesStarted: series(bars, "matches_started", "", barFrom, barTo, mStepMs, "sum"),
